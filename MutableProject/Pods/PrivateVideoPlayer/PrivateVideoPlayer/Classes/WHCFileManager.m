...
//将图片写入到本地
+ (BOOL)writeImageAtPath:(NSString *)folder content:(UIImage *)img imgName:(NSString*)imgName {
    NSString  * path =[WHCFileManager documentsDir];
    NSString *directryPath = [path stringByAppendingPathComponent:folder];
    NSFileManager *fileManager =[NSFileManager defaultManager];
    [fileManager createDirectoryAtPath:directryPath withIntermediateDirectories:YES attributes:nil error:nil];
    NSString *filePath = [directryPath stringByAppendingPathComponent:[NSString stringWithFormat:@"%@",[NSString stringWithFormat:@"%@.png",imgName]]]; // 保存文件的名称
